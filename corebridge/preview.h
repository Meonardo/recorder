...
public:
	Preview(QWidget* parent, Qt::WindowFlags flags = Qt::WindowFlags());
	~Preview();
   ...
   * byte boundary. */
	...
	void UpdateScale(float previewScale) { this->previewScale = previewScale; }
	void UpdatePosition(int previewX, int previewY) {
		this->previewX = previewX;
		this->previewY = previewY;
	}
	void UpdateSize(int previewCX, int previewCY) {
		this->previewCX = previewCX;
		this->previewCY = previewCY;
	}
	void UpdateDPI(float dpi) { this->dpi = dpi; }
	void SetDrawBox(gs_vertbuffer_t* drawBox) { this->drawBox = drawBox; }
...
private:
	...
	bool DrawSelectionBox(float x1, float y1, float x2, float y2, gs_vertbuffer_t* box);
	...
	float previewScale = 0.0f, dpi = 1.0f;
	gs_vertbuffer_t* drawBox = nullptr;
};
} // namespace core
