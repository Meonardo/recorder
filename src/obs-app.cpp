...
string CurrentTimeString() {
	...
	if (ratio_less<system_clock::period, seconds::period>::value && written &&
...
bool OBSApp::InitLocale() {
	...
	if (!userLocale && defaultLang) {
		for (auto& locale_ : GetPreferredLocales()) {
			...
			if (!locale.empty())
				QLocale::setDefault(
				  QLocale(QString::fromStdString(locale).replace('-', '_')));
...
#undef DEF_PALETTE_ASSIGN
	...
	case QPalette::Inactive: func((QPalette::ColorGroup)group); break;
	default:
...
static void ui_task_handler(obs_task_t task, void* param, bool wait) {
	...
	QMetaObject::invokeMethod(App(), "Exec", wait ? WaitConnection() : Qt::AutoConnection,
...
bool OBSApp::OBSInit() {
	...
	bool hideFromCapture =
	  config_get_bool(globalConfig, "BasicWindow", "HideOBSWindowsFromCapture");
	...
	connect(mainWindow, &MainWindow::destroyed, this, &OBSApp::quit);
...
static void delete_oldest_file(bool has_prefix, const char* location) {
	...
	unsigned int maxLogs =
	  (unsigned int)config_get_uint(App()->GlobalConfig(), "General", "MaxLogs");
...
string GetOutputFilename(const char* path, const char* container, bool noSpace, bool overwrite,
			 const char* format) {
	...
	if (!dir) {
		OBSMessageBox::warning(main, QTStr("Output.BadPath.Title"),
...
static int run_program(fstream& logFile, int argc, char* argv[]) {
	...
	try {
		...
		if (!multi) {
			...
			QPushButton* cancelButton =
			  mb.addButton(QTStr("Cancel"), QMessageBox::NoRole);
	...
	run:
		...
		if (crosDir) {
			...
			QMessageBox mb(QMessageBox::Critical, QTStr("ChromeOS.Title"),
				       QTStr("ChromeOS.Text"), buttons, nullptr);
		...
		if (unclean_shutdown && !safe_mode) {
			QMessageBox mb(QMessageBox::Warning, QTStr("AutoSafeMode.Title"),
		...
		qInstallMessageHandler(
		  [](QtMsgType type, const QMessageLogContext&, const QString& message) {
			  switch (type) {
#ifdef _DEBUG
			  case QtDebugMsg: blog(LOG_DEBUG, "%s", QT_TO_UTF8(message)); break;
			  case QtInfoMsg: blog(LOG_INFO, "%s", QT_TO_UTF8(message)); break;
#else
			  case QtDebugMsg:
			  case QtInfoMsg: break;
#endif
			  case QtWarningMsg: blog(LOG_WARNING, "%s", QT_TO_UTF8(message)); break;
			  case QtCriticalMsg:
			  case QtFatalMsg: blog(LOG_ERROR, "%s", QT_TO_UTF8(message)); break;
			  }
		  });
		...
		if (permissionsDialogLastShown < MACOS_PERMISSIONS_DIALOG_VERSION) {
			OBSPermissions check(nullptr, screen_permission, video_permission,
		...
		//#ifdef _WIN32
		//		if (IsRunningOnWine()) {
		//			QMessageBox mb(QMessageBox::Question, QTStr("Wine.Title"),
		//				       QTStr("Wine.Text"));
