...
typedef NTSTATUS(STDMETHODCALLTYPE* fn_NtMapViewOfSection)(HANDLE, HANDLE, PVOID, ULONG_PTR, SIZE_T,
							   PLARGE_INTEGER, PSIZE_T, SECTION_INHERIT,
							   ULONG, ULONG);

typedef NTSTATUS(STDMETHODCALLTYPE* fn_NtUnmapViewOfSection)(HANDLE, PVOID);

typedef NTSTATUS(STDMETHODCALLTYPE* fn_NtQuerySection)(HANDLE, SECTION_INFORMATION_CLASS, PVOID,
						       SIZE_T, PSIZE_T);
...
static NTSTATUS NtMapViewOfSection_hook(HANDLE SectionHandle, HANDLE ProcessHandle,
	...
	ret = ntMap(SectionHandle, ProcessHandle, BaseAddress, ZeroBits, CommitSize, SectionOffset,
	...
	if (ntQuery(SectionHandle, SectionBasicInformation, &section_information,
		    sizeof(section_information), &wrote) < 0)
		return ret;
...
void log_blocked_dlls(void) {
	for (int i = 0; i < _countof(blocked_modules); i++) {
		blocked_module_t* b = &blocked_modules[i];
		if (b->blocked_count) {
			blog(LOG_WARNING, "Blocked loading of '%S' %" PRIu64 " time%S.", b->name,
			     b->blocked_count, b->blocked_count == 1 ? L"" : L"s");
		}
	}
}
