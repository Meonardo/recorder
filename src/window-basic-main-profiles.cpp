...
static bool AskForProfileName(QWidget* parent, std::string& name, const char* title,
...
bool MainWindow::AddProfile(bool create_new, const char* title, const char* text,
			  const char* init_text, bool rename) {
	std::string name;

	bool showWizardChecked =
	  config_get_bool(App()->GlobalConfig(), "Basic", "ConfigOnNewProfile");

	if (!AskForProfileName(this, name, title, text, create_new, showWizardChecked, init_text))
		return false;

	return CreateProfile(name, create_new, showWizardChecked, rename);
}

bool MainWindow::CreateProfile(const std::string& newName, bool create_new, bool showWizardChecked,
			     bool rename) {
	std::string newDir;
	std::string newPath;
	ConfigFile config;
	...
	if (create_new) {
		config_set_bool(App()->GlobalConfig(), "Basic", "ConfigOnNewProfile",
				showWizardChecked);
	}
...
bool MainWindow::NewProfile(const QString& name) {
	return CreateProfile(name.toStdString(), true, false, false);
}

bool MainWindow::DuplicateProfile(const QString& name) {
	return CreateProfile(name.toStdString(), false, false, false);
}

void MainWindow::DeleteProfile(const char* profileName, const char* profileDir) {
...
void MainWindow::RefreshProfiles() {
	...
	auto addProfile = [&](const char* name, const char* path) {
		...
		connect(action, &QAction::triggered, this, &MainWindow::ChangeProfile);
...
void MainWindow::ResetProfileData() {
	ResetVideo();
	service = nullptr;
	InitService();
	ResetOutputs();
...
void MainWindow::on_actionDupProfile_triggered() {
	AddProfile(false, Str("AddProfile.Title"), Str("AddProfile.Text"));
}

void MainWindow::on_actionRenameProfile_triggered() {
	std::string curDir = config_get_string(App()->GlobalConfig(), "Basic", "ProfileDir");
	std::string curName = config_get_string(App()->GlobalConfig(), "Basic", "Profile");

	/* Duplicate and delete in case there are any issues in the process */
	bool success = AddProfile(false, Str("RenameProfile.Title"), Str("AddProfile.Text"),
				  curName.c_str(), true);
	if (success) {
...
void MainWindow::on_actionRemoveProfile_triggered(bool skipConfirmation) {
	std::string newName;
	std::string newPath;
	ConfigFile config;

	std::string oldDir = config_get_string(App()->GlobalConfig(), "Basic", "ProfileDir");
	std::string oldName = config_get_string(App()->GlobalConfig(), "Basic", "Profile");
	...
	if (!skipConfirmation) {
		QString text = QTStr("ConfirmRemove.Text").arg(QT_UTF8(oldName.c_str()));

		QMessageBox::StandardButton button =
		  OBSMessageBox::question(this, QTStr("ConfirmRemove.Title"), text);
	...
	blog(LOG_INFO, "Switched to profile '%s' (%s)", newName.c_str(), newDir);
	blog(LOG_INFO, "------------------------------------------------");
	...
	if (needsRestart) {
		QMessageBox::StandardButton button = OBSMessageBox::question(
		  this, QTStr("Restart"),
		  QTStr("LoadProfileNeedsRestart").arg(settingsRequiringRestart));
...
void MainWindow::on_actionImportProfile_triggered() {
	char path[512];
	...
	QString dir = SelectDirectory(this, QTStr("Basic.MainMenu.Profile.Import"), home);
...
void MainWindow::on_actionExportProfile_triggered() {
	char path[512];

	QString home = QDir::homePath();

	QString currentProfile =
	  QString::fromUtf8(config_get_string(App()->GlobalConfig(), "Basic", "ProfileDir"));
	...
	QString dir = SelectDirectory(this, QTStr("Basic.MainMenu.Profile.Export"), home);
...
void MainWindow::CheckForSimpleModeX264Fallback() {
