...
template<typename T>
inline size_t GetCallbackIdx(vector<OBSStudioCallback<T>>& callbacks, T callback,
			     void* private_data) {
...
struct OBSStudioAPI : obs_frontend_callbacks {
	...
	inline OBSStudioAPI(MainWindow* main_) : main(main_) {}
	...
	obs_source_t* obs_frontend_get_current_transition(void) override { return nullptr; }

	void obs_frontend_set_current_transition(obs_source_t* transition) override {}

	int obs_frontend_get_transition_duration(void) override { return 0; }
	...
	bool obs_frontend_add_scene_collection(const char* name) override {
		bool success = false;
		QMetaObject::invokeMethod(main, "AddSceneCollection", WaitConnection(),
	...
	void obs_frontend_create_profile(const char* name) override {
		QMetaObject::invokeMethod(main, "NewProfile", Q_ARG(QString, name));
	}
	...
	void* obs_frontend_add_tools_menu_qaction(const char* name) override { return nullptr; }

	void obs_frontend_add_tools_menu_item(const char* name, obs_frontend_cb callback,
					      void* private_data) override {}

	void* obs_frontend_add_dock(void* dock) override { return nullptr; }
	...
	bool obs_frontend_add_custom_qdock(const char* id, void* dock) override { return true; }
	...
	bool obs_frontend_preview_enabled(void) override { return true; }
	...
	char* obs_frontend_get_last_screenshot(void) override { return nullptr; }

	char* obs_frontend_get_last_replay(void) override { return nullptr; }
...
obs_frontend_callbacks* InitializeAPIInterface(MainWindow* main) {
