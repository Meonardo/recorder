...
static void HandleIntProperty(obs_property_t* prop, std::vector<int>& bitrates) {
	...
	for (int i = obs_property_int_min(prop); i <= max_; i += step) bitrates.push_back(i);
}
...
static void HandleSampleRate(obs_property_t* prop, const char* id) {
	...
	std::unique_ptr<obs_data_t, decltype(ReleaseData)> data{obs_encoder_defaults(id),
								ReleaseData};
...
static void HandleEncoderProperties(const char* id, std::vector<int>& bitrates) {
	...
	switch (type) {
	case OBS_PROPERTY_INT: return HandleIntProperty(bitrate, bitrates);

	case OBS_PROPERTY_LIST: return HandleListProperty(bitrate, id, bitrates);
...
static void PopulateBitrateLists() {
	...
	call_once(once, []() {
		...
		for (size_t i = 0; obs_enum_encoder_types(i, &id); i++) {
			...
			blog(LOG_DEBUG, "%s (%s) encoder bitrates:%s", EncoderName(id), id,
			     ss.str().c_str());
		}
...
static void PopulateSimpleAACBitrateMap() {
	...
	call_once(once, []() {
		...
		for (auto& entry : simpleAACBitrateMap)
			ss << "\n	" << setw(3) << entry.first << " kbit/s: '"
			   << EncoderName(entry.second) << "' (" << entry.second << ')';
...
int FindClosestAvailableAudioBitrate(const char* id, int bitrate) {
	...
	for (auto val :
	     encoderBitrates[encoder].empty() ? fallbackBitrates : encoderBitrates[encoder]) {
		if (next > val) {
			if (val == bitrate)
				return bitrate;
