...
static void OBSStopStreaming(void* data, calldata_t* params) {
	...
	QMetaObject::invokeMethod(output->main, "StreamingStop", Q_ARG(int, code),
...
void SimpleOutput::LoadRecordingPreset() {
	...
	const char* audio_encoder =
	  config_get_string(main->Config(), "SimpleOutput", "RecAudioEncoder");
...
SimpleOutput::SimpleOutput(MainWindow* main_) : BasicOutputHandler(main_) {
	...
	const char* audio_encoder =
	  config_get_string(main->Config(), "SimpleOutput", "StreamAudioEncoder");
	...
	if (!ffmpegOutput) {
		...
		if (useReplayBuffer) {
			...
			const char* str =
			  config_get_string(main->Config(), "Hotkeys", "ReplayBuffer");
			if (str)
...
void SimpleOutput::SetupVodTrack(obs_service_t* service) {
	...
	bool enableForCustomServer =
	  config_get_bool(GetGlobalConfig(), "General", "EnableCustomServerVodTrack");
...
bool SimpleOutput::ConfigureRecording(bool updateReplayBuffer) {
	...
	if (updateReplayBuffer) {
		...
		string strPath = GetRecordingFilename(path, ffmpegOutput ? "avi" : format, noSpace,
...
void AdvancedOutput::UpdateStreamSettings() {
	bool applyServiceSettings =
	  config_get_bool(main->Config(), "AdvOut", "ApplyServiceSettings");
	bool enforceBitrate = !config_get_bool(main->Config(), "Stream1", "IgnoreRecommended");
	...
	for (size_t i = 0; i < MAX_AUDIO_MIXES; i++) {
		...
		if (track == streamTrackIndex || track == vodTrackIndex) {
			if (applyServiceSettings) {
				...
				obs_service_apply_encoder_settings(main->GetService(), nullptr,
								   settings[i]);
...
inline void AdvancedOutput::SetupVodTrack(obs_service_t* service) {
	int streamTrack = config_get_int(main->Config(), "AdvOut", "TrackIndex");
...
bool AdvancedOutput::StartRecording() {
	...
	if (!ffmpegOutput || ffmpegRecording) {
		...
		string strPath = GetRecordingFilename(path, recFormat, noSpace, overwriteIfExists,
