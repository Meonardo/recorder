...
static void OBSStreamStarting(void* data, calldata_t* params) {
	...
	output->callback->OnStreamDelayStarting(sec);
}
...
static void OBSStreamStopping(void* data, calldata_t* params) {
	...
	int sec = (int)obs_output_get_active_delay(obj);
	if (sec == 0) {
		output->callback->OnStreamStopped("", 0);
	} else {
		output->callback->OnStreamDelayStopping(sec);
	}
}
...
static void OBSStartStreaming(void* data, calldata_t* /* params */) {
	...
	output->callback->OnStreamStarted();
}
...
static void OBSStopStreaming(void* data, calldata_t* params) {
	...
	output->callback->OnStreamStopped(last_error, code);
}
...
static void OBSStartRecording(void* data, calldata_t* /* params */) {
	...
	output->callback->OnRecordingStarted();
}
...
static void OBSRecordStopping(void* data, calldata_t* /* params */) {
	core::BasicOutputHandler* output = static_cast<core::BasicOutputHandler*>(data);
	output->callback->OnRecordingStopping();
}
...
static void OBSRecordFileChanged(void* data, calldata_t* params) {
	...
	output->callback->OnRecordingFileChanged(output->lastRecordingPath);
}
...
static void OBSStartReplayBuffer(void* data, calldata_t* /* params */) {
	...
	output->callback->OnReplayBufferStarted();
}
...
static void OBSStartVirtualCam(void* data, calldata_t* /* params */) {
	...
	output->callback->OnVirtualCamStarted();
}
...
static void OBSStopVirtualCam(void* data, calldata_t* params) {
	...
	output->callback->OnVirtualCamStopped("", code);
}
...
static const char* GetStreamOutputType(const obs_service_t* service) {
	...
	if (output) {
		...
		blog(LOG_WARNING, "The output '%s' is not registered, fallback to another one",
		     output);
	}
	...
	blog(LOG_WARNING, "No output compatible with the service '%s' is registered",
	     obs_service_get_id(service));
...
struct SimpleOutput : BasicOutputHandler {
	...
	SimpleOutput(OutputCallback* callback);
...
void SimpleOutput::LoadRecordingPreset_Lossy(const char* encoderId) {
	videoRecording =
	  obs_video_encoder_create(encoderId, "simple_video_recording", nullptr, nullptr);
	if (!videoRecording)
...
void SimpleOutput::LoadStreamingPreset_Lossy(const char* encoderId) {
	videoStreaming =
	  obs_video_encoder_create(encoderId, "simple_video_stream", nullptr, nullptr);
	if (!videoStreaming)
...
void SimpleOutput::LoadRecordingPreset() {
	...
	if (strcmp(quality, "Stream") == 0) {
		...
		if (strcmp(audio_encoder, "opus") == 0)
			success =
			  CreateSimpleOpusEncoder(audioRecording, 192, "simple_opus_recording", 0);
		else
			success =
			  CreateSimpleAACEncoder(audioRecording, 192, "simple_aac_recording", 0);
		...
		for (int i = 0; i < MAX_AUDIO_MIXES; i++) {
			...
			if (strcmp(audio_encoder, "opus") == 0) {
				...
				success = CreateSimpleOpusEncoder(audioTrack[i], GetAudioBitrate(),
								  name, i);
			} else {
				snprintf(name, sizeof name, "simple_aac_recording%d", i);
				success =
				  CreateSimpleAACEncoder(audioTrack[i], GetAudioBitrate(), name, i);
			}
			if (!success)
...
SimpleOutput::SimpleOutput(OutputCallback* callback) : BasicOutputHandler(callback) {
	...
	if (strcmp(audio_encoder, "opus") == 0)
		success =
		  CreateSimpleOpusEncoder(audioStreaming, GetAudioBitrate(), "simple_opus", 0);
	else
		success =
		  CreateSimpleAACEncoder(audioStreaming, GetAudioBitrate(), "simple_aac", 0);
	...
	if (strcmp(audio_encoder, "opus") == 0)
		success =
		  CreateSimpleOpusEncoder(audioArchive, GetAudioBitrate(), SIMPLE_ARCHIVE_NAME, 1);
	else
		success =
		  CreateSimpleAACEncoder(audioArchive, GetAudioBitrate(), SIMPLE_ARCHIVE_NAME, 1);
	...
	if (!ffmpegOutput) {
		...
		if (useReplayBuffer) {
			...
			replayBuffer =
			  obs_output_create("replay_buffer", Str("ReplayBuffer"), nullptr, hotkey);
			...
			replayBufferStopping.Connect(signal, "stopping", OBSReplayBufferStopping,
						     this);
			replayBufferSaved.Connect(signal, "saved", OBSReplayBufferSaved, this);
		}

		fileOutput =
		  obs_output_create("ffmpeg_muxer", "simple_file_output", nullptr, nullptr);
		if (!fileOutput)
	...
	stopRecording.Connect(obs_output_get_signal_handler(fileOutput), "stop", OBSStopRecording,
			      this);
	recordStopping.Connect(obs_output_get_signal_handler(fileOutput), "stopping",
...
void SimpleOutput::Update() {
	...
	obs_service_apply_encoder_settings(CoreApp->GetService(), videoSettings, audioSettings);
	...
	case VIDEO_FORMAT_P010: break;
	default: obs_encoder_set_preferred_video_format(videoStreaming, VIDEO_FORMAT_NV12);
	}
	...
	if (usingRecordingPreset) {
		if (ffmpegOutput) {
			...
			if (flv) {
				...
				for (int i = 0; i < MAX_AUDIO_MIXES; i++) {
					if ((tracks & (1 << i)) != 0) {
						obs_encoder_set_audio(audioTrack[i],
								      obs_get_audio());
					}
				}
			}
...
bool SimpleOutput::SetupStreaming(obs_service_t* service) {
	...
	if (outputType != type) {
		...
		startStreaming.Connect(obs_output_get_signal_handler(streamOutput), "start",
				       OBSStartStreaming, this);
		stopStreaming.Connect(obs_output_get_signal_handler(streamOutput), "stop",
...
void SimpleOutput::UpdateRecording() {
	...
	if (!ffmpegOutput) {
		...
		if (flv || strcmp(quality, "Stream") == 0) {
			...
			for (int i = 0; i < MAX_AUDIO_MIXES; i++) {
				if ((tracks & (1 << i)) != 0) {
					obs_output_set_audio_encoder(fileOutput, audioTrack[i],
								     idx++);
				}
			}
		}
	...
	if (replayBuffer) {
		...
		if (flv || strcmp(quality, "Stream") == 0) {
			...
			for (int i = 0; i < MAX_AUDIO_MIXES; i++) {
				if ((tracks & (1 << i)) != 0) {
					obs_output_set_audio_encoder(replayBuffer, audioTrack[i],
								     idx2++);
				}
			}
		}
...
bool SimpleOutput::ConfigureRecording(bool updateReplayBuffer) {
	...
	if (updateReplayBuffer) {
		...
		std::string strPath = GetRecordingFilename(path, ffmpegOutput ? "avi" : format,
							   noSpace, overwriteIfExists, f.c_str(),
							   ffmpegOutput);
...
bool SimpleOutput::StartReplayBuffer() {
	...
	if (!obs_output_start(replayBuffer)) {
		blog(LOG_ERROR, "start replay failed");
		return false;
	}
...
struct AdvancedOutput : BasicOutputHandler {
	...
	AdvancedOutput(OutputCallback* callback);
...
AdvancedOutput::AdvancedOutput(OutputCallback* cb) : BasicOutputHandler(cb) {
	...
	if (ffmpegOutput) {
		fileOutput =
		  obs_output_create("ffmpeg_output", "adv_ffmpeg_output", nullptr, nullptr);
		if (!fileOutput)
			throw "Failed to create recording FFmpeg output "
			      "(advanced output)";
	} else {
		...
		if (!useStreamEncoder) {
			videoRecording = obs_video_encoder_create(
			  recordEncoder, "advanced_video_recording", recordEncSettings, nullptr);
			if (!videoRecording)
	...
	videoStreaming = obs_video_encoder_create(streamEncoder, "advanced_video_stream",
	...
	const char* rate_control = obs_data_get_string(
	  useStreamEncoder ? streamEncSettings : recordEncSettings, "rate_control");
	if (!rate_control)
		rate_control = "";
	...
	recordStopping.Connect(obs_output_get_signal_handler(fileOutput), "stopping",
			       OBSRecordStopping, this);
	recordFileChanged.Connect(obs_output_get_signal_handler(fileOutput), "file_changed",
				  OBSRecordFileChanged, this);
}
...
void AdvancedOutput::UpdateStreamSettings() {
	...
	if (applyServiceSettings) {
		...
		obs_service_apply_encoder_settings(CoreApp->GetService(), settings, nullptr);
		if (!enforceBitrate) {
			blog(LOG_INFO, "User is ignoring service bitrate limits.");
...
inline void AdvancedOutput::SetupStreaming() {
	...
	const char* id = obs_service_get_id(CoreApp->GetService());
	if (strcmp(id, "rtmp_custom") == 0) {
		OBSDataAutoRelease settings = obs_data_create();
		obs_service_apply_encoder_settings(CoreApp->GetService(), settings, nullptr);
		obs_encoder_update(videoStreaming, settings);
	...
	if (!flv) {
		for (int i = 0; i < MAX_AUDIO_MIXES; i++) {
			if ((tracks & (1 << i)) != 0) {
				...
				if (replayBuffer)
					obs_output_set_audio_encoder(replayBuffer, recordTrack[i],
								     idx);
				idx++;
			}
		}
	...
	for (size_t i = 0; i < MAX_AUDIO_MIXES; i++) {
		...
		if (track == streamTrackIndex || track == vodTrackIndex) {
			if (applyServiceSettings) {
				...
				obs_service_apply_encoder_settings(CoreApp->GetService(), nullptr,
								   settings[i]);
...
std::string BasicOutputHandler::GetRecordingFilename(const char* path, const char* container,
