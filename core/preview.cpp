...
Preview::Preview(QWidget* parent, Qt::WindowFlags flags) : OBSQTDisplay(parent, flags) {
...
vec2 Preview::GetMouseEventPos(QMouseEvent* event) {
	float pixelRatio = dpi;
...
static bool FindItemAtPos(obs_scene_t* /* scene */, obs_sceneitem_t* item, void* param) {
	...
	if (CloseFloat(pos3.x, pos3_.x) && CloseFloat(pos3.y, pos3_.y) &&
	    transformedPos.x >= 0.0f && transformedPos.x <= 1.0f && transformedPos.y >= 0.0f &&
...
vec3 Preview::GetSnapOffset(const vec3& tl, const vec3& br) {
...
OBSSceneItem Preview::GetItemAtPos(const vec2& pos, bool selectBelow) {
...
bool Preview::SelectedAtPos(const vec2& pos) {
	OBSScene scene = CoreApp->GetCurrentScene();
	if (!scene)
		return false;
...
void Preview::GetStretchHandleData(const vec2& pos, bool ignoreGroup) {
	OBSScene scene = CoreApp->GetCurrentScene();
	if (!scene)
		return;

	float scale = previewScale / dpi;
	vec2 scaled_pos = pos;
...
void Preview::keyReleaseEvent(QKeyEvent* event) {
...
void Preview::mousePressEvent(QMouseEvent* event) {
	...
	if (altDown || shiftDown || ctrlDown) {
		...
		obs_scene_enum_items(CoreApp->GetCurrentScene(), FindSelected, &data);
	...
	mousePos = startPos;
	wrapper = obs_scene_save_transform_states(CoreApp->GetCurrentScene(), true);
	changed = false;
}

void Preview::UpdateCursor(uint32_t& flags) {
...
void Preview::DoSelect(const vec2& pos) {
	OBSScene scene = CoreApp->GetCurrentScene();
...
void Preview::DoCtrlSelect(const vec2& pos) {
...
void Preview::ProcessClick(const vec2& pos) {
	Qt::KeyboardModifiers modifiers = QGuiApplication::keyboardModifiers();
...
void Preview::SnapItemMovement(vec2& offset) {
	OBSScene scene = CoreApp->GetCurrentScene();
...
void Preview::MoveItems(const vec2& pos) {
	Qt::KeyboardModifiers modifiers = QGuiApplication::keyboardModifiers();
	OBSScene scene = CoreApp->GetCurrentScene();
...
static bool IntersectLine(float x1, float x2, float x3, float x4, float y1, float y2, float y3,
			  float y4) {
...
bool Preview::FindSelected(obs_scene_t*, obs_sceneitem_t* item, void* param) {
...
void Preview::BoxItems(const vec2& startPos, const vec2& pos) {
	OBSScene scene = CoreApp->GetCurrentScene();
	if (!scene)
		return;
...
vec3 Preview::CalculateStretchPos(const vec3& tl, const vec3& br) {
...
void Preview::ClampAspect(vec3& tl, vec3& br, vec2& size, const vec2& baseSize) {
...
void Preview::SnapStretchingToScreen(vec3& tl, vec3& br) {
	uint32_t stretchFlags = (uint32_t)stretchHandle;
...
void Preview::CropItem(const vec2& pos) {
...
void Preview::StretchItem(const vec2& pos) {
	Qt::KeyboardModifiers modifiers = QGuiApplication::keyboardModifiers();
	obs_bounds_type boundsType = obs_sceneitem_get_bounds_type(stretchItem);
	uint32_t stretchFlags = (uint32_t)stretchHandle;
	bool shiftDown = (modifiers & Qt::ShiftModifier);
...
void Preview::RotateItem(const vec2& pos) {
	OBSScene scene = CoreApp->GetCurrentScene();
	Qt::KeyboardModifiers modifiers = QGuiApplication::keyboardModifiers();
	bool shiftDown = (modifiers & Qt::ShiftModifier);
	bool ctrlDown = (modifiers & Qt::ControlModifier);
...
#define ROT_SNAP(rot, thresh)                      \
	...
	if (shiftDown) {
		for (int i = 0; i <= 360 / 15; i++) { ROT_SNAP(i * 15 - 90, 7.5); }
	} else if (!ctrlDown) {
...
void Preview::leaveEvent(QEvent*) {
	std::lock_guard<std::mutex> lock(selectMutex);
	if (!selectionBox)
		hoveredPreviewItems.clear();
}
...
static void DrawRotationHandle(gs_vertbuffer_t* circle, float rot, float pixelRatio, bool invert) {
	...
	gs_matrix_translate3f(-HANDLE_RADIUS * 1.5 * pixelRatio, -HANDLE_RADIUS * 1.5 * pixelRatio,
			      0.0f);
...
static void DrawStripedLine(float x1, float y1, float x2, float y2, float thickness, vec2 scale) {
	...
	for (int i = 0, l = ceil(dist / 15); i < l; i++) {
		...
		if (x1 < x2) {
			dx = min(xx1 + 7.5f * offX, x2);
		} else {
			dx = max(xx1 + 7.5f * offX, x2);
		}

		if (y1 < y2) {
			dy = min(yy1 + 7.5f * offY, y2);
		} else {
			dy = max(yy1 + 7.5f * offY, y2);
		}
...
bool Preview::DrawSelectedOverflow(obs_scene_t*, obs_sceneitem_t* item, void* param) {
	if (obs_sceneitem_locked(item))
		return true;
	...
	Preview* prev = reinterpret_cast<Preview*>(param);
	...
	while (gs_effect_loop(solid, "Draw")) { gs_draw_sprite(prev->overflow, 0, 1, 1); }
...
bool Preview::DrawSelectedItem(obs_scene_t*, obs_sceneitem_t* item, void* param) {
	if (obs_sceneitem_locked(item))
		return true;
	...
	QColor selColor = QColor::fromRgb(255, 0, 0);
	QColor cropColor = QColor::fromRgb(0, 255, 0);
	QColor hoverColor = QColor::fromRgb(0, 127, 255);
...
#undef DRAW_SIDE
	...
	gs_load_vertexbuffer(prev->drawBox);
	gs_effect_set_vec4(colParam, &red);
...
bool Preview::DrawSelectionBox(float x1, float y1, float x2, float y2, gs_vertbuffer_t* rectFill) {
	float pixelRatio = dpi;
...
void Preview::DrawSceneEditing() {
	...
	if (selectionBox) {
		...
		DrawSelectionBox(startPos.x * previewScale, startPos.y * previewScale,
				 mousePos.x * previewScale, mousePos.y * previewScale, rectFill);
	}
...
void Preview::ResetScrollingOffset() {
	vec2_zero(&scrollingOffset);
}

void Preview::SetScalingLevel(int32_t newScalingLevelVal) {
...
void Preview::SetScalingAmount(float newScalingAmountVal) {
...
static void RenderSpacingHelper(int sourceIndex, vec3& start, vec3& end, vec3& viewport,
	...
	vec3 labelMargin;
	vec3_set(&labelMargin, SPACER_LABEL_MARGIN * pixelRatio, SPACER_LABEL_MARGIN * pixelRatio,
		 1.0f);
...
void Preview::DrawSpacingHelpers() {
	if (locked)
		return;
	...
	if (parentGroup) {
		...
		matrix4_scale3f(&boxTransform, &boxTransform, groupOti.scale.x, groupOti.scale.y,
				1.0f);
		matrix4_rotate_aa4f(&boxTransform, &boxTransform, 0.0f, 0.0f, 1.0f,
				    RAD(groupOti.rot));
		matrix4_translate3f(&boxTransform, &boxTransform, groupOti.pos.x, groupOti.pos.y,
				    0.0f);
	}
	...
	// Init viewport
	vec3 viewport;
	vec3_set(&viewport, previewCX, previewCY, 1.0f);
...
void Preview::ClampScrollingOffsets() {
	obs_video_info ovi;
	obs_get_video_info(&ovi);
